...
window.addEventListener('message', function(event) {
    ...
    console.log('event-S01->',event);
    onMessageCallback(event.data);
    console.log('event.data-S02->',event.data);
});
...
function onMessageCallback(data) {
    ...
    if (data.sourceId && screenCallback) {
        console.log('S03-data.sourceId->',data.sourceId);
        console.log('S04-screenCallback->',screenCallback);
...
function getSourceId(callback) {
    console.log('sourceId-S05->',sourceId);
    if (!callback) throw '"callback" parameter is mandatory.';
    if(sourceId) return callback(sourceId);
    
    screenCallback = callback;

    console.log('S06-screenCallback->',screenCallback);
    console.log('S07-callback->',callback);
...
function getChromeExtensionStatus(extensionid, callback) {
    ...
    if (arguments.length != 2) {
        ...
        extensionid = 'pjjengflmiaafllmcbejjdmagplkgjpo'; // default extension-id
    }
...
function getScreenConstraints(callback) {
    console.log('S09-callback->',callback);
    ...
    console.log('chromeMediaSource-S10->: ',chromeMediaSource);
    console.log('sourceId-S11->: ',sourceId);
